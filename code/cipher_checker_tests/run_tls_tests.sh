...
function runWithURL {
  podman run --name "cipher_checker_tests_$1" -e "CHROMIUM_URL=$2" --replace cipher_checker_tests
  podman cp cipher_checker_tests_$job:/app/result.json ./results/$1.json
}
